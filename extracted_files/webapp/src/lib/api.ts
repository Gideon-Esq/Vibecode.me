...
export const musicApi = {
  search: (query: string) =>
    api.get<YouTubeTrack[]>(`/api/youtube/search?q=${encodeURIComponent(query)}`),

  getTrending: () => api.get<YouTubeTrack[]>("/api/youtube/trending"),

  getRelated: (videoId: string) =>
    api.get<YouTubeTrack[]>(`/api/youtube/related/${videoId}`),

  getArtist: (name: string) =>
    api.get<ArtistProfile>(`/api/youtube/artist?name=${encodeURIComponent(name)}`),
};
